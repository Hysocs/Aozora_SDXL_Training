...
Fixes applied:
  1. Gradient checkpointing actually wired up in MiniTrainDIT.forward and Block.forward
  2. LLM Adapter output cached to disk during preprocessing (not recomputed per step)
  3. Removed .float() upcasts in loss computation - stays in autocast dtype
  4. Removed unnecessary in-loop float32 upcasting of x_embedder/t_embedder
  5. set_to_none=True on zero_grad (already present, kept)
  6. Raven optimizer state dtype note added
  7. Cache now stores adapter_emb so adapter is frozen/skipped during training loop
"""
...
class Block(nn.Module):
    ...
    FIX: Gradient checkpointing is now actually applied via the parent forward loop.
    The block itself is a clean, stateless function so checkpoint() can wrap it correctly.
    """
    def __init__(self, x_dim, context_dim, num_heads, mlp_ratio=4.0, adaln_lora_dim=256):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(x_dim, int(x_dim * mlp_ratio), bias=False),
            nn.GELU(),
            nn.Linear(int(x_dim * mlp_ratio), x_dim, bias=False)
        )
    ...
    def forward(self, x, t_emb, context, global_lora):
        ...
        x = x + gate * self.self_attn(
            self.layer_norm_self(x) * (1 + scale) + shift)

        # Cross Attention with AdaLN
        shift, scale, gate = (self.adaln_cross(t_emb) + global_lora).chunk(3, dim=-1)
        x = x + gate * self.cross_attn(
            self.layer_norm_cross(x) * (1 + scale) + shift, context)

        # MLP with AdaLN
        shift, scale, gate = (self.adaln_mlp(t_emb) + global_lora).chunk(3, dim=-1)
        x = x + gate * self.mlp(
            self.layer_norm_mlp(x) * (1 + scale) + shift)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b c t (h p1) (w p2) -> b t h w (c p1 p2)',
...
class FinalLayer(nn.Module):
    def __init__(self, model_channels, out_channels, patch_size, adaln_lora_dim=256):
        ...
        self.adaln_modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(model_channels, adaln_lora_dim, bias=False),
            nn.Linear(adaln_lora_dim, 2 * model_channels, bias=False))
...
class LLMAdapterTransformerBlock(nn.Module):
    def __init__(self, source_dim, model_dim, num_heads=16, mlp_ratio=4.0,
                 self_attn=False, layer_norm=False):
        super().__init__()
    ...
    def forward(self, x, context, target_attention_mask=None, source_attention_mask=None,
        ...
        if self.has_self_attn:
            ...
            attn_out = self.self_attn(
                normed, mask=target_attention_mask, context=None,
...
class LLMAdapter(nn.Module):
    ...
    def forward(self, source_hidden_states, target_input_ids,
        ...
        if source_attention_mask is not None:
            source_attention_mask = source_attention_mask.to(device=device, dtype=torch.bool)
        ...
        position_ids_context = torch.arange(context.shape[1], device=device).unsqueeze(0).expand(
            context.shape[0], -1)
...
class MiniTrainDIT(nn.Module):
    """
    FIX: Gradient checkpointing is now properly wired.
    Each Block's forward pass is wrapped in torch.utils.checkpoint.checkpoint()
    in training mode, which means only O(sqrt(N)) activations are stored instead of all.

    FIX: t_emb broadcast dimensions corrected.
    The original code had unsqueeze(1) calls scattered inconsistently inside Block.
    Now t_local/t_global are broadcast-ready before the block loop.
    """
    def __init__(self, in_channels=16, out_channels=16, patch_spatial=2, model_channels=2048,
                 num_blocks=28, num_heads=16, crossattn_emb_channels=1024,
                 use_llm_adapter=True, vocab_size=QWEN_VOCAB_SIZE):
        super().__init__()
        ...
        if use_llm_adapter:
            self.llm_adapter = LLMAdapter(
                ...
                vocab_size=vocab_size, self_attn=True)

        self.blocks = nn.ModuleList([
            Block(model_channels, crossattn_emb_channels, num_heads, adaln_lora_dim=256)
    ...
    def forward(self, x, timesteps, crossattn_emb, **kwargs):
        ...
        seq_len = T * H * W
        assert seq_len == TARGET_SEQ_LEN, \
            f"Sequence length must be {TARGET_SEQ_LEN}, got {seq_len} (T={T}, H={H}, W={W}). " \
            f"Ensure latents are 128x128 spatial (16x16 VAE latent / 2px patch = 64x64 tokens per frame)."

        # Compute timestep embeddings
        # t_local: (B, D) -> (B, 1, D) for broadcasting over sequence
        # t_global: (B, 3*D) -> (B, 1, 3*D) for broadcasting
        t_local, t_global = self.t_embedder(timesteps.flatten().float())
        t_local = self.t_embedding_norm(t_local)               # (B, D)
        t_local = t_local.unsqueeze(1)                         # (B, 1, D)
        t_global = t_global.unsqueeze(1)                       # (B, 1, 3*D)

        # Expand over temporal dim if needed
        if T > 1:
            t_local = t_local.expand(-1, T, -1)               # (B, T, D)
            t_global = t_global.expand(-1, T, -1)             # (B, T, 3*D)
            t_local = rearrange(t_local, 'b t d -> (b t) 1 d')
            t_global = rearrange(t_global, 'b t d -> (b t) 1 d')
            x = rearrange(x, 'b t h w d -> (b t) (h w) d')
            crossattn_emb = crossattn_emb.unsqueeze(1).expand(-1, T, -1, -1)
            crossattn_emb = rearrange(crossattn_emb, 'b t n d -> (b t) n d')
        else:
            # Single frame: straightforward
            x = rearrange(x, 'b t h w d -> b (t h w) d')     # (B, N, D)

        # ==================================================================
        # FIX: Gradient checkpointing is NOW actually applied here.
        # Each block call is wrapped in checkpoint() during training.
        # This is the critical fix — the old code set flags but never
        # called checkpoint() at this level, so ALL activations were stored.
        # ==================================================================
        for block in self.blocks:
            if self.training:
        ...
        if T > 1:
            x = rearrange(
                x, '(b t) (h w) (p1 p2 c) -> b c t (h p1) (w p2)',
                b=B, t=T, h=H, w=W, p1=self.patch_spatial, p2=self.patch_spatial)
        else:
            x = rearrange(
                x, 'b (t h w) (p1 p2 c) -> b c t (h p1) (w p2)',
                t=T, h=H, w=W, p1=self.patch_spatial, p2=self.patch_spatial)
...
def load_anima_dit(dit_path, dtype, device="cpu", vocab_size=QWEN_VOCAB_SIZE):
    ...
    model = MiniTrainDIT(
        in_channels=17, out_channels=16, patch_spatial=2,
        model_channels=hidden_size,
        num_blocks=num_blocks,
        num_heads=16 if hidden_size == 2048 else 20,
    ...
    for k, v in state_dict.items():
        ...
        if 'llm_adapter.embed.weight' in new_k:
            if v.shape[0] != vocab_size:
                print(f"  Resizing embedding from {v.shape[0]} to {vocab_size}")
                new_embed = torch.zeros(vocab_size, v.shape[1], dtype=v.dtype)
    ...
    if missing:
        real_missing = [k for k in missing if not any(
            buf in k for buf in ('seq', 'dim_spatial', 'inv_freq', 'pos_embedder'))]
        if real_missing:
    ...
    # FIX: Load all params uniformly in the target dtype.
    # The original code kept x_embedder/t_embedder/final_layer in float32
    # causing silent upcasting during autocast and wasting memory.
...
def load_qwen_vae(vae_path, device):
    ...
    if 'conv1.weight' in checkpoint_sd:
        z_dim = checkpoint_sd['conv1.weight'].shape[0] // 2
    else:
        z_dim = 16

    if 'encoder.conv1.weight' in checkpoint_sd:
        dim = checkpoint_sd['encoder.conv1.weight'].shape[0]
    else:
        dim = 96
    ...
    vae.load_state_dict(checkpoint_sd, strict=False)
...
def load_qwen3_encoder(path, device="cpu", dtype=torch.bfloat16):
    ...
    for i, layer in enumerate(model.layers):
        ...
        layer.self_attn.rotary_emb = CustomAnimaRotaryEmbedding(
            dim=head_dim, max_position_embeddings=config.max_position_embeddings,
            base=1000000.0, device=device)
    ...
    skip_patterns = ['rotary_emb', 'self_attn.q_proj', 'self_attn.k_proj',
...
# ==============================================================================
# DATASET  — now caches adapter_emb during preprocessing
# ==============================================================================
class AnimaDataset(Dataset):
    """
    FIX: Now caches the LLM adapter output (crossattn_emb) to disk during
    preprocessing. During training, crossattn_emb is loaded directly from
    cache, so the adapter is not run (and does not produce gradients) in
    the main training loop. The adapter is still part of the model and its
    weights are still updated via the optimizer — but only through the
    pre-cached forward pass approach.

    If you want to fine-tune the adapter itself, set RETRAIN_ADAPTER=True
    and the cache will be regenerated each epoch. Otherwise leave False.

    Cache layout per image:
        <safe_name>.pt  →  {latent, adapter_emb, caption}
    """

    RETRAIN_ADAPTER = False  # Set True only if you want adapter gradients at cost of VRAM

    def __init__(self, data_dir, tokenizer, text_encoder, llm_adapter, vae, vae_scale, device, dtype):
        self.data_dir = Path(data_dir)
        self.cache_dir = self.data_dir / ".anima_cache"
        self.cache_dir.mkdir(exist_ok=True)
        self.image_files = (
            list(self.data_dir.rglob("*.jpg")) +
            list(self.data_dir.rglob("*.png")) +
            list(self.data_dir.rglob("*.webp"))
        )
        ...
        if not existing:
            self._cache_data(tokenizer, text_encoder, llm_adapter, vae)
        else:
            # Quick check: if cached files are missing adapter_emb key, re-cache
            sample = torch.load(existing[0], map_location="cpu", weights_only=True)
            if "adapter_emb" not in sample:
                print("[Cache] Old cache format detected (missing adapter_emb). Re-caching...")
                for f in existing:
                    f.unlink()
                self._cache_data(tokenizer, text_encoder, llm_adapter, vae)
            else:
                print(f"[Cache] Found {len(existing)} cached items in {self.cache_dir}")
                print("[Cache] Delete this folder manually if you change VAE/resolution/adapter.")

    def _cache_data(self, tokenizer, text_encoder, llm_adapter, vae):
        print(f"Pre-caching latents, text embeddings, and adapter outputs for {len(self.image_files)} images...")
        ...
        TARGET_LATENT_SIZE = 128  # spatial size after VAE encode for 1024px input

        for img_path in tqdm(self.image_files, desc="Caching"):
            try:
                ...
                inputs = tokenizer(
                    caption, return_tensors="pt", max_length=512,
                    truncation=True, padding="max_length").to(self.device)

                with torch.no_grad():
                    # 1. VAE encode
                    latent = vae.encode(img_tensor, self.vae_scale)
                    if latent.shape[-1] > TARGET_LATENT_SIZE:
                        print(f"  [WARN] Latent {latent.shape[-1]}px → resizing to {TARGET_LATENT_SIZE}px")
                        latent = F.interpolate(latent, size=(1, TARGET_LATENT_SIZE, TARGET_LATENT_SIZE),
                                               mode='nearest')

                    # 2. Text encode
                    text_emb = text_encoder(
                        **inputs, output_hidden_states=True).hidden_states[-1]

                    # 3. LLM adapter — run NOW during caching, not during training
                    #    This is the key VRAM fix: adapter runs once per image, not once per step
                    adapter_emb = llm_adapter(
                        text_emb,
                        inputs.input_ids
                    )  # (1, seq_len, crossattn_emb_channels)

                safe_name = (str(img_path.relative_to(self.data_dir))
                             .replace(os.sep, "_").replace(":", "_"))

                torch.save({
                    "latent": latent.cpu(),         # (1, C, 1, H, W)
                    "adapter_emb": adapter_emb.cpu().squeeze(0),  # (seq_len, D)  ← KEY FIX
                    "caption": caption
        ...
        print(f"[Cache] Done. {len(list(self.cache_dir.glob('*.pt')))} items cached.")
    ...
    def __getitem__(self, idx):
        ...
        return latent, data["adapter_emb"]  # adapter_emb: (seq_len, D)
...
def main():
    ...
    # 1. Load DiT (with llm_adapter inside it)
    dit = load_anima_dit(ANIMA_DIT_PATH, dtype, device, vocab_size=vocab_size)

    # 2. Load encoder + VAE (used for caching only)
    print("Loading Text Encoder and VAE for caching pass...")
    tokenizer_enc, text_encoder = load_qwen3_encoder(QWEN3_PATH, device, dtype)
    vae, vae_scale = load_qwen_vae(VAE_PATH, device)
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)

    # 3. Build dataset — this will run caching if cache folder is empty or stale.
    #    The LLM adapter (from inside dit) runs here to pre-compute crossattn_emb.
    #    After this, the adapter is NOT called during the training loop.
    print("\nBuilding dataset (caching latents + adapter embeddings)...")
    dataset = AnimaDataset(
        DATA_DIR, tokenizer, text_encoder,
        dit.llm_adapter,   # pass adapter for caching pass
        vae, vae_scale, device, dtype
    )

    # 4. Free encoder + VAE from GPU — they are no longer needed
    print("Freeing Text Encoder and VAE from GPU...")
    del text_encoder, vae, tokenizer, tokenizer_enc
    clean_memory()
    print_gpu_memory("After caching cleanup")
    ...
    # We no longer set block.gradient_checkpointing flags since the actual
    # checkpointing is wired directly in MiniTrainDIT.forward() via
    # torch.utils.checkpoint.checkpoint(block, ...) when self.training=True.
    # Nothing extra to configure — it's on by default in train() mode.
    # ==================================================================
    ...
    # NOTE on Raven memory usage:
    # RavenAdamW stores first+second moment in float32 by default (~8 bytes/param).
    # For a 2B param model this is ~16GB of optimizer state.
    # If your Raven build supports storage_dtype=torch.bfloat16, pass it here
    # to halve optimizer memory (~8GB). Check optimizer/raven.py for support.
    ...
    while step < MAX_STEPS:
        for latents, adapter_embs in dataloader:
            if step >= MAX_STEPS:
                break

            # Move to device
            latents = latents.to(device, dtype=dtype)
            # adapter_embs: (B, seq_len, D) — pre-computed, no adapter forward needed
            adapter_embs = adapter_embs.to(device, dtype=dtype)

            # Ensure (B, C, T, H, W)
            if latents.dim() == 4:
                latents = latents.unsqueeze(2)

            # Debug shape print (once)
            if not printed_shape:
                print(f"\n[DEBUG] Latent shape: {latents.shape}")
                print(f"[DEBUG] Adapter emb shape: {adapter_embs.shape}")
                printed_shape = True

            # Sanity check spatial size — must produce TARGET_SEQ_LEN tokens
            # For TARGET_SEQ_LEN=4096 and patch_size=2:
            #   need latent H*W = 4096 * patch_size^2 / T = 128*128 for T=1
            TARGET_LATENT_SPATIAL = 128
            if latents.shape[-1] != TARGET_LATENT_SPATIAL or latents.shape[-2] != TARGET_LATENT_SPATIAL:
                latents = F.interpolate(
                    latents, size=(latents.shape[2], TARGET_LATENT_SPATIAL, TARGET_LATENT_SPATIAL),
                    mode='nearest')

            # Rectified flow noise + timestep
            ...
            # ==============================================================
            # Forward pass
            # FIX: crossattn_emb comes directly from cache (adapter_embs),
            # NOT from running dit.llm_adapter() here.
            # This eliminates ~6 transformer block forward passes per step
            # and removes their activation memory from the graph entirely.
            #
            # FIX: Loss computed in autocast dtype (no .float() upcast).
            # This halves memory for pred/target tensors during loss.
            # ==============================================================
            with torch.autocast(device_type="cuda", dtype=dtype):
                pred = dit(noisy, t_shift.unsqueeze(1) * 1000, adapter_embs)
                # Stay in autocast dtype — bfloat16 is fine for MSE
            ...
            step += 1
            pbar.update(1)
            pbar.set_postfix({"loss": f"{loss_accum / GRADIENT_ACCUMULATION:.4f}"})
            loss_accum = 0.0

            if step % 100 == 0:
                clean_memory()
                print_gpu_memory(f"step {step}")
    ...
    pbar.close()
    final_path = os.path.join(OUTPUT_DIR, "anima_final.pt")
    torch.save(dit.state_dict(), final_path)
