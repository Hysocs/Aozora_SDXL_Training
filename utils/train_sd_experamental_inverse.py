...
def generate_character_map(pil_image):
    ...
    lab_image = cv2.cvtColor(bilateral, cv2.COLOR_BGRLAB)
...
class TrainingConfig:
    def __init__(self):
        ...
        if not hasattr(self, 'VIRTUAL_TIMESTEP_MAX'):
            self.VIRTUAL_TIMESTEP_MAX = 2000 # 0-1000 Standard, 1000-2000 Inverse/Recon
        ...
        if not hasattr(self, 'CLIP_GRAD_NORM'):
            self.CLIP_GRAD_NORM = 25.0
        if not hasattr(self, 'INVERSE_TASK_PROBABILITY'):
            self.INVERSE_TASK_PROBABILITY = 0.25
        
        # --- REPLACED: INPAINTING MODE ---
        # We implicitly enable inpainting logic for steps 1000-2000 now
...
def main():
    ...
    print("INFO: Inpainting Mode ENABLED for timesteps 1000-2000.")
    print("      (Progressive Pixel Dropout: Inverted Difficulty)")
    ...
    while not done:
        for batch in dataloader:
            ...
            with torch.autocast(device_type=device.type, dtype=config.compute_dtype, enabled=True):
                ...
                split_step = config.STANDARD_TIMESTEP_MAX 
                mask_std = timesteps < split_step 
                mask_inv = timesteps >= split_step 
                ...
                if mask_std.any():
                    ...
                    per_pixel_loss_std = F.mse_loss(pred_std.float(), target_std.to(pred_std.dtype).float(), reduction="none")
                    if getattr(config, 'LOSS_TYPE', 'Default') == "Semantic":
                        if original_images and None not in original_images:
                            b, c, h, w = noisy_latents_std.shape
                            ...
                            loss_std = (per_pixel_loss_std * modulation).mean()
                        else:
                            loss_std = per_pixel_loss_std.mean()
                    else:
                        loss_std = per_pixel_loss_std.mean()
                    loss_val += loss_std
                    
                if mask_inv.any():
                    current_latents = latents[mask_inv]
                    current_timesteps = timesteps[mask_inv]
                    b_inv, c_inv, h_inv, w_inv = current_latents.shape
                    
                    step_min = config.STANDARD_TIMESTEP_MAX 
                    step_max = config.VIRTUAL_TIMESTEP_MAX  
                    progress = (current_timesteps - step_min).float() / (step_max - step_min)
                    
                    is_4x4 = progress < 0.25
                    is_2x2 = (progress >= 0.25) & (progress < 0.50)
                    is_1x1 = progress >= 0.50
                    
                    rand_map = torch.rand((b_inv, 1, h_inv, w_inv), device=device, dtype=current_latents.dtype)
                    
                    if is_4x4.any():
                        cnt_4x4 = is_4x4.sum()
                        small_4x4 = torch.rand((cnt_4x4, 1, h_inv // 4, w_inv // 4), device=device, dtype=current_latents.dtype)
                        rand_map[is_4x4] = F.interpolate(small_4x4, size=(h_inv, w_inv), mode='nearest')

                    if is_2x2.any():
                        cnt_2x2 = is_2x2.sum()
                        small_2x2 = torch.rand((cnt_2x2, 1, h_inv // 2, w_inv // 2), device=device, dtype=current_latents.dtype)
                        rand_map[is_2x2] = F.interpolate(small_2x2, size=(h_inv, w_inv), mode='nearest')
                        
                    keep_probs = 0.01 + (0.98 * progress)
                    
                    keep_probs_tensor = keep_probs.view(-1, 1, 1, 1).to(device=device, dtype=current_latents.dtype)

                    mask = (rand_map < keep_probs_tensor).to(dtype=current_latents.dtype)
                    masked_input = current_latents * mask

                    pred_reconstruct = unet(masked_input, timesteps[mask_inv], embeds[mask_inv],
                                       added_cond_kwargs={"text_embeds": pooled[mask_inv], "time_ids": time_ids[mask_inv]}).sample

                    raw_loss = F.mse_loss(pred_reconstruct.float(), current_latents.float(), reduction="none")
                    ...
                    if getattr(config, 'LOSS_TYPE', 'Default') == "Semantic":
                        if original_images and None not in original_images:
                            ...
                            modulation = 1.0 + (semantic_map * config.SEMANTIC_LOSS_STRENGTH)
                            reconstruct_loss = (raw_loss * modulation).mean()
                        else:
                            reconstruct_loss = raw_loss.mean()
                    else:
                        reconstruct_loss = raw_loss.mean()

                    loss_val += (reconstruct_loss * config.INVERSE_LOSS_SCALE)
